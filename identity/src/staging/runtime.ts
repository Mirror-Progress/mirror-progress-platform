...
export async function assertStagingRuntime(db: Pool): Promise<void> {
  const { rows } = await db.query(`SELECT current_user::text AS role,session_user::text AS login,
    ...
    EXISTS (SELECT 1 FROM unnest(ARRAY['mirror_principal','mirror_schema_migration','mirror_staging_operator',
      'mirror_staging_reconciliation','mirror_staging_approval','mirror_staging_audit','mirror_staging_recovery']) AS targets(name)
    ...
    (has_function_privilege(current_user,'mirror_staging_approve(text,bigint,text)','EXECUTE') OR
      has_function_privilege(current_user,'mirror_staging_approve_recovery(text,text)','EXECUTE') OR
