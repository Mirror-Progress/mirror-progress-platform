...
import { STAGING_PROFILE, type HostedProfile } from "./staging-config.js";
...
export function stagingContainerEnvironment(input: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return hostedContainerEnvironment(input, STAGING_PROFILE);
}
export function productionContainerEnvironment(input: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return hostedContainerEnvironment(input, PRODUCTION);
}
function hostedContainerEnvironment(input: NodeJS.ProcessEnv, profile: HostedProfile): NodeJS.ProcessEnv {
  assertRuntimeEnvironment(input);
  if (input.IDENTITY_MODE !== profile.mode || input.IDENTITY_TRANSPORT !== "alb" || input.IDENTITY_OWNER_CREDENTIALS) throw new Error("staging_runtime_only");
  ...
  if (c.username !== profile.runtimeRole || c.dbname !== profile.database || c.engine !== "postgres" ||
  ...
  const url = new URL(`postgresql://${host}:5432/${profile.database}`);
  url.username = profile.runtimeRole; url.password = c.password;
