...
try {
  const env = stagingContainerEnvironment(process.env);
  ...
  const cert = spawnSync("openssl", ["req", "-x509", "-newkey", "ec", "-pkeyopt", "ec_paramgen_curve:P-256", "-nodes",
    "-days", "7", "-subj", "/CN=accounts.staging.mirrorprogress.com", "-keyout", env.IDENTITY_TLS_KEY_FILE!,
...
} catch { console.error("Identity staging startup failed; inspect configuration without logging secrets."); process.exitCode = 1; }
