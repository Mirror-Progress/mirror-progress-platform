...
async function main() {
  if (process.env.IDENTITY_MODE !== 'staging') throw new Error('Staging only');
  const url = process.env.IDENTITY_OPERATOR_DATABASE_URL ?? ''; stagingDatabase(url, false);
  ...
  try {
    ...
    console.info('Assisted staging recovery action audited. No email sent; factor reenrollment is required.');
  } catch (error) {
