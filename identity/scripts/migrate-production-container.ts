...
try {
  ...
  if (c.username !== "mirror_identity_production_owner" || typeof c.password !== "string" || c.password.length < 24 ||
      c.host !== process.env.IDENTITY_DATABASE_HOST || c.port !== 5432 || c.engine !== "postgres" ||
      c.dbname !== "mirror_identity_production") throw new Error("invalid_owner_credentials");
  const input = { ...process.env }; delete input.IDENTITY_OWNER_CREDENTIALS;
  const env = productionContainerEnvironment(input), url = new URL(env.DATABASE_URL!);
  ...
  await import("./migrate-production.js");
} catch { console.error("Identity production migration bootstrap failed; no credentials logged."); process.exitCode = 1; }
