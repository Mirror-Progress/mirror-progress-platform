...
const Form: React.FC = () => {
  ...
  const hidePopup = () => {
    if (popup.current) {
      gsap.fromTo(
        '#popup',
        { display: 'flex', top: '0%' },
        { display: 'none', top: '100%', ease: 'power3.inOut' }
      );
      setEmailValue('');
      setMessageValue('');
  ...
  // Updated handleForm: sends data to the API route and then shows the popup.
  const handleForm = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!isValidEmail(emailValue) || messageValue === '') return;
      ...
      <div className="h-full w-full relative flex justify-center items-center ">
        <form action="" className="w-full" onSubmit={(e) => handleForm(e)}>
          ...
          >
            ...
            />
            <p
              className={`font-diatype uppercase text-[12px] text-[#FF9500] leading-100 tracking-m3p py-[12px] ${
                isValidEmail(emailValue) === false && emailValue !== ''
                  ? 'opacity-1'
                  : 'opacity-0'
              }`}
            >
        ...
        >
          <div className="w-[340px] mb-[84px]">
            ...
            <p className="font-diatype text-[18px] max-md:text-[16px] leading-120 text-center mt-[64px]">
              Thank you for your message. We’ve received it and will get back to you as soon as we can.
            </p>
          </div>
          <div>
            <button
        ...
        <div className="w-full overflow-y-scroll scrollbar-hide flex justify-center">
          <div className="md:max-w-[464px] max-md:max-w-[313px]">
            ...
            <div className="pb-[30px]">
              {policyText.map((t) => (
                <div key={t.id} className="font-dmSans text-[17px] max-md:text-[12px] leading-120 font-normal mb-[30px]">
                  <h4>{t.title}</h4>
                  <p>{t.text}</p>
