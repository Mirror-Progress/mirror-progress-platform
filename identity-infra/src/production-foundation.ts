...
export interface ProductionFoundationConfig {
  stage: 'production'; account: string; region: string; vpcId: string;
...
export function validateProductionFoundationConfig(config: ProductionFoundationConfig) {
  if (config.stage !== 'production' || !/^\d{12}$/.test(config.account) || config.region !== 'us-east-1' ||
      ...
      config.availabilityZones.some(zone => !new RegExp(`^${config.region}[a-z]$`).test(zone))) throw new Error('invalid_production_foundation_config');
}

/** Additive independent stateful foundation. No service, DNS, Cognito, email or cutover. */
export class ProductionIdentityFoundation extends Stack {
  ...
  constructor(scope: Construct, id: string, config: ProductionFoundationConfig, props: StackProps = {}) {
    ...
    Tags.of(this).add('Application', 'MirrorIdentity'); Tags.of(this).add('Environment', 'production');
    ...
    this.repository = new ecr.Repository(this, 'ServiceImages', { repositoryName: 'mirror-identity-production',
    ...
    this.database = new rds.DatabaseInstance(this, 'Database', { vpc, vpcSubnets: { subnetType: ec2.SubnetType.PRIVATE_ISOLATED },
      engine, parameterGroup: parameters, databaseName: 'mirror_identity_production',
      instanceType: ec2.InstanceType.of(ec2.InstanceClass.T4G, ec2.InstanceSize.SMALL),
      credentials: rds.Credentials.fromGeneratedSecret('mirror_identity_production_owner', { encryptionKey: secretsKey,
        secretName: 'mirror-identity/production/database-owner' }),
    ...
    this.runtimeCredentials = secret('RuntimeCredentials', 'Least-privilege production SQL login; not the database owner',
      { username: 'mirror_identity_production_runtime', dbname: 'mirror_identity_production', engine: 'postgres', port: 5432 });
