...
export class ProductionIdentityService extends Stack {
  ...
  constructor(scope: Construct, id: string, config: ProductionServiceConfig, foundation: ProductionIdentityFoundation, props: StackProps = {}) {
    ...
    const cluster = new ecs.Cluster(this, 'Cluster', { vpc, clusterName: 'mirror-identity-production', containerInsightsV2: ecs.ContainerInsights.ENABLED });
    ...
    const container = task.addContainer('Identity', { image, user: '1000:1000', readonlyRootFilesystem: true,
      command: ['node', 'dist/scripts/start-production-container.js'], environment, secrets,
    ...
    const acl = new waf.CfnWebACL(this, 'EdgeRateLimit', { scope: 'REGIONAL', defaultAction: { allow: {} },
      visibilityConfig: { cloudWatchMetricsEnabled: true, metricName: 'MirrorIdentityProduction', sampledRequestsEnabled: false },
